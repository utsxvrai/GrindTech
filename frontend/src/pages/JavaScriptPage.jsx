...
export default function JavaScriptPage() {
  ...
  useEffect(() => {
    ...
    const fetchTopics = async () => {
        try {
              // Always use 'JavaScript' for tech name
              const techResponse = await api.get('/tech/name/JavaScript');
  ...
  return (
    <div className="h-screen w-screen bg-black text-white relative overflow-hidden font-sans selection:bg-javascript-gold/30 flex flex-col">
      {/* Animated gradient background */}
      <div className="absolute inset-0 bg-gradient-to-br from-javascript-gold/10 via-black/50 to-black pointer-events-none z-0" />

      {/* Floating orbs background */}
      <div className="absolute inset-0 pointer-events-none z-0 overflow-hidden">
        <div className="absolute top-20 left-10 w-64 h-64 bg-javascript-gold/20 rounded-full blur-3xl animate-float" />
        <div className="absolute bottom-20 right-10 w-80 h-80 bg-javascript-gold/10 rounded-full blur-3xl animate-float" style={{ animationDelay: '2s' }} />
        <div className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 w-96 h-96 bg-javascript-gold/5 rounded-full blur-3xl animate-float" style={{ animationDelay: '4s' }} />
      </div>
        ...
        <div className="max-w-[1600px] mx-auto px-6 h-20 flex items-center justify-between">
          <div className="flex items-center gap-8">
            ...
            <div className="flex items-center gap-3">
              <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-javascript-gold to-yellow-600 flex items-center justify-center shadow-[0_0_20px_rgba(247,223,30,0.4)]">
                <svg xmlns="http://www.w3.org/2000/svg" width="28" height="28" viewBox="0 0 24 24" fill="none">
                  <text x="12" y="16" textAnchor="middle" fill="currentColor" fontSize="11" fontWeight="bold" fontFamily="Arial, sans-serif">JS</text>
                </svg>
              </div>
              <div className="flex flex-col">
                <span className="text-xl font-bold tracking-tight leading-none">
                  Grind<span className="text-javascript-gold">JavaScript</span>
                </span>
          ...
          <div className="flex items-center gap-4">
            ...
            >
              <Zap className="w-4 h-4 text-javascript-gold" />
              Contribute
            </button>
              ...
              >
                <div className="text-right hidden md:block">
                  ...
                  <div className="flex items-center justify-end gap-1.5">
                    <span className={`text-[10px] font-bold px-1.5 py-0.5 rounded ${isPro ? 'bg-javascript-gold text-black' : 'bg-gray-800 text-gray-400'}`}>
                      {isPro ? 'PRO' : 'FREE'}
                    </span>
                    {!isPro && (
                      <button
                        onClick={() => navigate('/payment')}
                        className="text-[10px] font-bold text-javascript-gold hover:underline"
                      >
                        UPGRADE
                      </button>
                    )}
          ...
          <div className="flex-grow flex w-[60%] flex-col min-h-0 overflow-hidden">
            ...
            <div className="shrink-0 mb-6">
              {!showMeter ? (
                 <div className="h-24 flex items-center justify-center">
                   ...
                   >
                     MASTER JAVASCRIPT LEVEL BY LEVEL.
                   </motion.p>
                 </div>
              ) : (
                <div className="px-2">
                  <LearningMeter techId={techId} accentColor="javascript-gold" />
                </div>
              )}
            </div>
            ...
            <div className="flex-grow overflow-y-auto pr-2 pb-32 scrollbar-hide">
              {fetchedTopics.length === 0 ? (
                <TechPageLoader accentColor="javascript-gold" />
              ) : (
      ...
      {fetchedTopics.map((module, index) => {
        ...
        <button
          ...
          className="lg:hidden fixed bottom-6 right-6 z-[60] w-14 h-14 rounded-full bg-javascript-gold text-black shadow-[0_0_20px_rgba(247,223,30,0.4)] flex items-center justify-center hover:scale-110 active:scale-95 transition-all"
        >
        ...
        {showMobileFeed && (
          ...
          >
            <div className="flex items-center justify-between p-4 border-b border-white/10 bg-zinc-900/50 backdrop-blur-md">
              <div className="flex items-center gap-2">
                <div className="w-2 h-2 rounded-full bg-javascript-gold animate-pulse" />
                <span className="font-bold text-sm tracking-widest text-white uppercase">JavaScript Feed</span>
              </div>
              <button

...
export default function TypeScriptPage() {
  ...
  useEffect(() => {
    ...
    const fetchTopics = async () => {
        try {
              // Always use 'TypeScript' for tech name
              const techResponse = await api.get('/tech/name/TypeScript');
  ...
  return (
    <div className="h-screen w-screen bg-black text-white relative overflow-hidden font-sans selection:bg-typescript-blue/30 flex flex-col">
      {/* Animated gradient background */}
      <div className="absolute inset-0 bg-gradient-to-br from-typescript-blue/10 via-black/50 to-black pointer-events-none z-0" />

      {/* Floating orbs background */}
      <div className="absolute inset-0 pointer-events-none z-0 overflow-hidden">
        <div className="absolute top-20 left-10 w-64 h-64 bg-typescript-blue/20 rounded-full blur-3xl animate-float" />
        <div className="absolute bottom-20 right-10 w-80 h-80 bg-typescript-blue/10 rounded-full blur-3xl animate-float" style={{ animationDelay: '2s' }} />
        <div className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 w-96 h-96 bg-typescript-blue/5 rounded-full blur-3xl animate-float" style={{ animationDelay: '4s' }} />
      </div>
        ...
        <div className="max-w-[1600px] mx-auto px-6 h-20 flex items-center justify-between">
          <div className="flex items-center gap-8">
            ...
            <div className="flex items-center gap-3">
              <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-typescript-blue to-blue-800 flex items-center justify-center shadow-[0_0_20px_rgba(49,120,198,0.4)]">
                <svg xmlns="http://www.w3.org/2000/svg" width="28" height="28" viewBox="0 0 24 24" fill="none">
                  <text x="12" y="16" textAnchor="middle" fill="currentColor" fontSize="11" fontWeight="bold" fontFamily="Arial, sans-serif">TS</text>
                </svg>
              </div>
              <div className="flex flex-col">
                <span className="text-xl font-bold tracking-tight leading-none">
                  Grind<span className="text-typescript-blue">TypeScript</span>
                </span>
          ...
          <div className="flex items-center gap-4">
            ...
            >
              <Zap className="w-4 h-4 text-typescript-blue" />
              Contribute
            </button>
              ...
              >
                <div className="text-right hidden md:block">
                  ...
                  <div className="flex items-center justify-end gap-1.5">
                    <span className={`text-[10px] font-bold px-1.5 py-0.5 rounded ${isPro ? 'bg-typescript-blue text-black' : 'bg-gray-800 text-gray-400'}`}>
                      {isPro ? 'PRO' : 'FREE'}
                    </span>
                    {!isPro && (
                      <button
                        onClick={() => navigate('/payment')}
                        className="text-[10px] font-bold text-typescript-blue hover:underline"
                      >
                        UPGRADE
                      </button>
                    )}
          ...
          <div className="flex-grow flex w-[60%] flex-col min-h-0 overflow-hidden">
            ...
            <div className="shrink-0 mb-6">
              {!showMeter ? (
                 <div className="h-24 flex items-center justify-center">
                   ...
                   >
                     MASTER TYPESCRIPT LEVEL BY LEVEL.
                   </motion.p>
                 </div>
              ) : (
                <div className="px-2">
                  <LearningMeter techId={techId} accentColor="typescript-blue" />
                </div>
              )}
            </div>
            ...
            <div className="flex-grow overflow-y-auto pr-2 pb-32 scrollbar-hide">
              {fetchedTopics.length === 0 ? (
                <TechPageLoader accentColor="typescript-blue" />
              ) : (
      ...
      {fetchedTopics.map((module, index) => {
        ...
        <button
          ...
          className="lg:hidden fixed bottom-6 right-6 z-[60] w-14 h-14 rounded-full bg-typescript-blue text-black shadow-[0_0_20px_rgba(49,120,198,0.4)] flex items-center justify-center hover:scale-110 active:scale-95 transition-all"
        >
        ...
        {showMobileFeed && (
          ...
          >
            <div className="flex items-center justify-between p-4 border-b border-white/10 bg-zinc-900/50 backdrop-blur-md">
              <div className="flex items-center gap-2">
                <div className="w-2 h-2 rounded-full bg-typescript-blue animate-pulse" />
                <span className="font-bold text-sm tracking-widest text-white uppercase">TypeScript Feed</span>
              </div>
              <button
